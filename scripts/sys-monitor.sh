...
case "$1" in
    start)
        ...
        sudo systemctl start vmagent 2>/dev/null || true
        echo "  VictoriaMetrics: http://localhost:8428"
        echo "  Node metrics:    http://localhost:9100/metrics"
        ;;
    stop)
        echo ">> Stopping monitoring..."
        sudo systemctl stop victoriametrics prometheus-node-exporter vmagent 2>/dev/null || true
        echo "  All monitoring stopped."
        ;;
    status)
        echo ">> Monitoring status:"
        for svc in victoriametrics prometheus-node-exporter vmagent vault; do
